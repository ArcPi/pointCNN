...
def main():
    ...
    parser.add_argument('--path_train', '-t', default='./data/modelnet/train_files.txt',help='Path to data')
    parser.add_argument('--path_val', '-v', default='./data/modelnet/test_files.txt',help='Path to validation data')
    parser.add_argument('--load_ckpt', help='Path to a check point file to load')
    # parser.add_argument('--setting', '-x', help='Setting to use', default='modelnet')
    parser.add_argument('--save_folder', '-s', default='./save_folder/',
    ...
    parser.add_argument('--epochs', help='Num of train epochs', type=int)
    ...
    if not args.no_timestamp_folder:
        ...
        root_folder = os.path.join(args.save_folder,
                                   '%s_%s_%d' % (args.model, time_string, os.getpid()))
    else:
    ...
    num_epochs = args.epochs or modelnet.num_epochs
    batch_size = args.batch_size or modelnet.batch_size
    step_val = modelnet.step_val
    rotation_range = modelnet.rotation_range
    rotation_range_val = modelnet.rotation_range_val
    scaling_range = modelnet.scaling_range
    scaling_range_val = modelnet.scaling_range_val
    jitter = modelnet.jitter
    jitter_val = modelnet.jitter_val
    pool_setting_val = None if not hasattr(modelnet, 'pool_setting_val') else modelnet.pool_setting_val
    pool_setting_train = None if not hasattr(modelnet, 'pool_setting_train') else modelnet.pool_setting_train

    # Load dataset
    print('{} load dataset'.format(datetime.now()))
    data_train, label_train, data_val, label_val = modelnet.load_fn(args.path_train, args.path_val)

    if modelnet.balance_fn is not None:
        num_train_before_balance = data_train.shape[0]
        repeat_num = modelnet.balance_fn(label_train)
    ...
    if modelnet.save_ply_fn is not None:
        ...
        if modelnet.map_fn is None:
            data_sample = data_train[:sample_num_for_ply]
        else:
            data_sample_list = []
            for idx in range(sample_num_for_ply):
                data_sample_list.append(modelnet.map_fn(data_train[idx], 0)[0])
            data_sample = np.stack(data_sample_list)
        modelnet.save_ply_fn(data_sample, folder)
    ...
    rotations = tf.placeholder(tf.float32, shape=(None, 3, 3), name='roatations')
    jitter_range = tf.placeholder(tf.float32, shape=(1), name='jitter_range')
    global_step = tf.Variable(0, trainable=False, name='global_step')
    is_training = tf.placeholder(tf.bool, name='is_training')

    data_train_placeholder = tf.placeholder(tf.float32, shape=data_train.shape, name='data_train')
    label_train_placeholder = tf.placeholder(tf.int32, shape=label_train.shape, name='label_train')
    data_val_palceholder = tf.placeholder(tf.float32, shape=data_val.shape, name='data_val')
    label_val_placceholder = tf.placeholder(tf.int32, shape=label_val.shape, name='label_val')
    ...
    if modelnet.map_fn is not None:
        dataset_train = dataset_train.map(lambda data, label:
                                          tuple(tf.py_func(modelnet.map_fn, [data, label], [tf.float32, label.dtype])),
                                          num_parallel_calls=modelnet.num_parallel_calls)
    if modelnet.keep_remainder:
    ...
    dataset_val = tf.data.Dataset.from_tensor_slices((data_val, label_val))
    if modelnet.map_fn is not None:
        dataset_val = dataset_val.map(lambda data, label: tuple(tf.py_func(
            modelnet.map_fn, [data, label], [tf.float32, label.dtype])), num_parallel_calls=modelnet.num_parallel_calls)
    if modelnet.keep_remainder:
    ...
    print('{} {:d} tesing batches per test'.format(datetime.now(), batch_num_val))

    # Define a feedable iterator
    ...
    if modelnet.data_dim > 3:
        points_sampled, features_sampled = tf.split(pts_fts_sampled,
                                                    [3, modelnet.data_dim - 3],
                                                    axis=-1,
                                                    name='split_points_features')
        if modelnet.use_extra_features:
            if modelnet.with_normal_feature:
                if modelnet.data_dim < 6:
                    print('Only 3D normals are supported!')
                    exit()
                elif modelnet.data_dim == 6:
                    features_augmented = pf.augment(features_sampled, rotations)
                else:
                    normals, rest = tf.split(features_sampled, [3, modelnet.data_dim - 6])
    ...
    net = model.Net(points=points_augmented, features=features_augmented, is_training=is_training, setting=modelnet)
    logits = net.logits
